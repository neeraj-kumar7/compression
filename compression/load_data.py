...
def load_data(path, detrac = False, train = False):
    ...
    if detrac:
        print( '\n DETRAC .... ')
        c = 0
        for i in os.listdir('./../datasets/Insight-MVT_Annotation_Train/'):
            n_data = []
            c+=1
            for j in tqdm(range(1, len(os.listdir('./../datasets/Insight-MVT_Annotation_Train/' + i + '/'))+1)):
                j = str(j)
                jj= 5-len(j)
                k = "img" + jj*"0" +j +".jpg"
                img = imageio.imread('./../datasets/Insight-MVT_Annotation_Train/' + i + '/' + k + '/')
                #print("image", img)
    ...
    data_load = data_array.transpose(0,3,1,2)
